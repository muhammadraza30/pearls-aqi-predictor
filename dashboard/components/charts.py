...
Light-themed, interactive charts for AQI trends and pollutant analysis.
"""
...
def create_aqi_trend_chart(df: pd.DataFrame, days: int = 30) -> go.Figure:
    ...
    # AQI threshold lines
    thresholds = [(50, "#00e400", "Good"), (100, "#d4a017", "Moderate"),
...
def create_weather_vs_aqi(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        height=380,
        **LIGHT_TEMPLATE,
        xaxis=dict(gridcolor="rgba(0,0,0,0.06)", title="Temperature (°C)"),
        yaxis=dict(gridcolor="rgba(0,0,0,0.06)", title="AQI"),
    )
    return fig
